        ...
        while True:
            try:
                ...
                if nccl2_trainer_id == 0:
                    ...
                    #  save middle state in test stage
                    # if steps in [7750, 9000, 17731]:
                    #     save_path = os.path.join(args.checkpoints,
                    #                              "step_" + str(steps))
                    #     fluid.io.save_persistables(exe, save_path, train_program)
                    if steps % args.validation_steps == 0 or last_epoch != current_epoch:
                        ...
                        if args.do_val:
                            evaluate_Info = evaluate_wrapper(args, reader, exe, test_prog,
                                             test_pyreader, graph_vars,
                                             current_epoch, steps)
                            # save middle state after 5000 step, and save the better result only
                            import re
                            #sample msg: s = 'ave loss: 1.067206, ave_acc: 0.437675, f1: 0.215873, data_num: 1789, elaps'
                            current_f1 = float(re.findall(r"f1: 0\.\d*,", evaluate_Info)[0][4:-1])
                            if current_f1 > max_f1:
                                if steps >= 5000:
                                    ...
                                    fluid.io.save_persistables(exe, save_path, train_program)
                                max_f1 = current_f1
                            log.info("current max_f1 " + str(max_f1) + " --------------------------------------------------------------")
