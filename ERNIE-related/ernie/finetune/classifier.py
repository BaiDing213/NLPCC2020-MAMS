...
def evaluate_classify(exe,
    ...
    qids, labels, scores, preds, logits = [], [], [], [], []
    ...
    while True:
        try:
            if use_multi_gpu_test:
                np_loss, np_acc, np_probs, np_labels, np_num_seqs, np_qids, np_logits= exe.run(
            ...
            logits.extend(np_logits.tolist())
            if np_qids is None:
                np_qids = np.array([])
    ...
    import pickle
    with open("/home/dutir923/zhoufengqing/paddle-pretrain/ERNIE-develop/eval/atsa_test_logit_ernie_large", "wb") as file:
